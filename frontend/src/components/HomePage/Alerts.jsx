...
import "./Alerts.css"; // Ensure this file exists and contains your styles
  ...
  useEffect(() => {
    const fetchAlerts = async (latitude, longitude) => {
      try {
        ...
        console.error("Error fetching alert data:", err.response || err.message);
        ...
        <div className="space-y-4">
          <h2 className="text-2xl font-bold text-red-600">
            <lord-icon
              ...
              trigger="hover"
              colors="primary:#121331,secondary:#c71f16,tertiary:#e8b730"
            ></lord-icon>
            Disaster Alerts
          </h2>
          {alertData.result.map((event, index) => (
            <div
              key={index}
              className="p-4 border border-red-600 bg-red-100 text-red-800 rounded"
            >
              <p>
                <strong>Event:</strong> {event.event_name || "N/A"}
              </p>
              <p>
                <strong>Description:</strong> {event.details?.description || "N/A"}
              </p>
              <p>
                <strong>Severity:</strong> {event.details?.severity || "N/A"}
              </p>
              <p>
                <strong>Urgency:</strong> {event.details?.urgency || "N/A"}
              </p>
              <p>
                <strong>Instruction:</strong> {event.details?.instruction || "N/A"}
              </p>
              <p>
                <strong>Sender:</strong> {event.details?.sender || "N/A"}
              </p>
              <p>
                <strong>Status:</strong> {event.details?.event_status || "N/A"}
              </p>
              <p>
                <strong>Certainty:</strong> {event.details?.certainty || "N/A"}
              </p>
            </div>
          ))}
        </div>
